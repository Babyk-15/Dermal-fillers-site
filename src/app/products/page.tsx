        ...
        <section className="section section--dark" aria-labelledby="products-cta-heading">
          <div className="container" style={{ textAlign: 'center' }}>
            <h2
              ...
              style={{ color: 'var(--color-white)', marginBottom: 'var(--space-4)', maxWidth: 480, marginInline: 'auto' }}
            >
              Ready to Order?
            </h2>
            <p
...
function ProductCard({
  ...
  return (
    <article
      className={`product-card${animate ? ' product-card--animate' : ''}`}
      style={
        expanded
          ? { boxShadow: 'var(--shadow-lg)', borderColor: 'rgba(196,135,154,0.40)' }
          : {}
      }
    >
        ...
        {p.lidocaine && (
          <span
            style={{
              ...
              background: 'rgba(196,135,154,0.12)',
      ...
      <div style={{ padding: 'var(--space-4) var(--space-5)' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: 'var(--space-3)' }}>
          {p.treatmentAreas.slice(0, 3).map((a) => (
            <span key={a} className="spec-pill" style={{ fontSize: '0.65rem' }}>
              {a}
            </span>
          ))}
          {p.treatmentAreas.length > 3 && (
            <span className="spec-pill" style={{ fontSize: '0.65rem', opacity: 0.7 }}>
              +{p.treatmentAreas.length - 3} more
            </span>
          )}
        </div>
        ...
        {expanded && (
          ...
          >
            <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-muted)', lineHeight: 1.65 }}>{p.description}</p>
            {p.ingredients.length > 0 && (
              <div>
                ...
                {p.ingredients.map((ing) => (
                  <p key={ing} style={{ fontSize: 'var(--text-xs)', color: 'var(--color-slate-700)', lineHeight: 1.6 }}>
                    · {ing}
                  </p>
                ))}
              </div>
            ...
            >
              <p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)' }}>
                <strong style={{ color: 'var(--color-champagne)' }}>CE · MDR 2017/745</strong> — Full compliance
      ...
      <div className="product-card__footer">
        <div>
          <span
            style={{
              fontSize: p.price ? 'var(--text-lg)' : 'var(--text-xs)',
              fontFamily: p.price ? 'var(--font-display)' : 'inherit',
              fontWeight: p.price ? 700 : 400,
              color: p.price ? 'var(--color-plum)' : 'var(--color-text-muted)',
            }}
          >
            {p.price ?? 'Price on enquiry'}
          </span>
          <span style={{ display: 'block', fontSize: '0.65rem', color: 'var(--color-text-muted)', marginTop: '1px' }}>
            {p.volume}
          </span>
        </div>
